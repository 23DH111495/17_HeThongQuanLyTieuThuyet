...
namespace WebNovel.Controllers
{
    public class BookController : Controller
    {
        ...
        //}
        [HttpPost]
        public JsonResult ToggleBookmarkAjax(int novelId)
        {
            // 1. Kiểm tra đăng nhập
            if (Session["UserId"] == null)
            {
                return Json(new { success = false, requireLogin = true, message = "Bạn cần đăng nhập!" });
            }
            ...
            try
            {
                ...
                // 2. Lấy ReaderId (Quan trọng)
                var reader = db.Readers.FirstOrDefault(r => r.UserId == userId);
                if (reader == null)
                {
                    return Json(new { success = false, message = "Lỗi: Tài khoản này chưa có thông tin Độc giả (Reader ID)." });
                }
                int currentReaderId = reader.Id;

                // 3. Xử lý Bookmark
                var existingBookmark = db.Bookmarks.FirstOrDefault(b => b.NovelId == novelId && b.ReaderId == currentReaderId);
                var novel = db.Novels.Find(novelId);
                ...
                if (existingBookmark == null)
                {
                    ...
                    if (novel != null) novel.BookmarkCount++;
                    isBookmarked = true;
                }
                else
                {
                    ...
                    if (novel != null && novel.BookmarkCount > 0) novel.BookmarkCount--;
                    isBookmarked = false;
                }
