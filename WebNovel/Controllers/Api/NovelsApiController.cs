                ...
                switch (preference.ToLower())
                {
                    case "popular":
                        ...
                        var randomPop = allNovels
                            .Except(popular)
                            .OrderBy(x => {
                                int novelSeed = (userId ?? 0) + DateTime.Now.Date.GetHashCode() + x.Id;
                                return new Random(novelSeed).Next();
                            })
                            .ToList();
                    ...
                    default:
                        ...
                        var random = allNovels
                            .Except(top.Concat(recent))
                            .OrderBy(x => {
                                int novelSeed = (userId ?? 0) + DateTime.Now.Date.GetHashCode() + x.Id;
                                return new Random(novelSeed).Next();
                            })
                            .ToList();
                        orderedNovels = top.Concat(recent).Concat(random)
                            .OrderBy(x => {
                                int novelSeed = (userId ?? 0) + DateTime.Now.Date.GetHashCode() + x.Id;
                                return new Random(novelSeed).Next();
                            })
