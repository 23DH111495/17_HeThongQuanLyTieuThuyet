...
class NovelDetailActivity : AppCompatActivity() {
    ...
    private fun getLoggedInReaderId(): Int {
        val sharedPref = getSharedPreferences("UserRating", MODE_PRIVATE)
        return sharedPref.getInt("readerId", 0) // 0 nghĩa chưa login
    }
    ...
    private fun submitRating(value: Int) {
        val readerId = getLoggedInReaderId()
        if (readerId == 0) {
        ...
        api.createOrUpdateRating(rating).enqueue(object : retrofit2.Callback<Map<String, Any>> {
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    Toast.makeText(this@NovelDetailActivity, "Rating successfully!", Toast.LENGTH_SHORT).show()
                    loadNovelRating()
                } else {
                    Toast.makeText(this@NovelDetailActivity, "Failed to submit rating", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onFailure(call: retrofit2.Call<Map<String, Any>>, t: Throwable) {
                Toast.makeText(this@NovelDetailActivity, "Error: ${t.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
        ...
        api.getAverageByNovel(novelId).enqueue(object : retrofit2.Callback<Map<String, Any>> {
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    ...
                    tvNovelRating.text = "★ $avg ($total ratings)"
                } else {
                    tvNovelRating.text = "★ 0 (0 ratings)"
                }
            }
