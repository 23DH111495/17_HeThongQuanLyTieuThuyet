...
class NovelDetailActivity : AppCompatActivity() {
    ...
    private var currentBookmarkId: Int? = null
    ...
    private var novelId: Int = 0 // Sẽ được lấy từ Intent

    private val IMAGE_PICK_CODE = 1000
    private val IMAGE_PICK_CODE_REPLY = 1001
    private val allComments = mutableListOf<CommentResponse>()

    // 🔹 CONSTANTS CHO BOOKMARK (FIXED) - Bỏ const vì R.* không phải compile-time constant
    companion object {
        private val ICON_BOOKMARK_FILLED = R.drawable.ic_star
        private val ICON_BOOKMARK_OUTLINE = R.drawable.icon_rating
        private val COLOR_BOOKMARKED = R.color.ranking_green_label
        private val COLOR_UNBOOKMARKED = R.color.ranking_text_secondary
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (currentUserId == null) {
            ...
            Log.d("NovelDetailActivity", "Using fallback userId: $currentUserId")
        }

        Log.d("NovelDetailActivity", "Final currentUserId: $currentUserId")
        ...
        try {
            ...
            Log.e("NovelDetailActivity", "Toolbar setup failed", e)
        }

        // 🔹 Khởi tạo API service (ĐÃ SỬA LỖI 401 & "Too many arguments")

        // 1. Dùng Client CÔNG KHAI (không token) cho API xem chi tiết truyện
        apiService = ApiClient.getClient().create(NovelApiService::class.java)

        // 2. Dùng Client RIÊNG TƯ (có token) cho API bình luận
        commentApiService = ApiClient.getAuthenticatedClient().create(CommentApiService::class.java)

        // 3. Dùng Client RIÊNG TƯ (có token) cho API bookmark
        ...
        // 🔹 Load bookmark status nếu đã đăng nhập
        if (isUserLoggedIn()) {
            loadBookmarkStatus()
        }
    }
    ...
    private fun getUserIdFromToken(): Int? {
        ...
        Log.d("NovelDetailActivity", "Token exists: ${token != null}")
        if (token == null) {
            return null
        }
        return try {
            ...
            if (parts.size != 3) {
                Log.e("NovelDetailActivity", "Invalid token format")
                return null
            }
            val payload = String(android.util.Base64.decode(parts[1], android.util.Base64.URL_SAFE))
            val jsonObject = JSONObject(payload)
            Log.d("NovelDetailActivity", "Token payload: $payload")
            val userId = when {
                ...
                jsonObject.has("uid") -> jsonObject.getInt("uid")
                    ...
                    Log.e("NovelDetailActivity", "No userId field found in token")
                    Log.e("NovelDetailActivity", "Available keys: ${jsonObject.keys().asSequence().toList()}")
                    null
                }
            }
            Log.d("NovelDetailActivity", "Extracted userId: $userId")
            userId
        } catch (e: Exception) {
            Log.e("NovelDetailActivity", "Error decoding token", e)
            e.printStackTrace()
            null
        }
    }

    // 🔹 Helper method kiểm tra đăng nhập
    private fun isUserLoggedIn(): Boolean {
        // SỬA LỖI: Chỉ cần kiểm tra currentUserId.
        // ID này đã được lấy từ Token trong onCreate.
        return currentUserId != null
    }

    // 🔹 Helper method lấy readerId
    // XÓA BỎ HÀM NÀY: Hàm này sử dụng SharedPreferences đã cũ và gây ra lỗi.
    ...
    private fun setupClickListeners() {
        ...
        // 🔹 FIXED: Setup bookmark button
        fabBookmark.setOnClickListener {
            if (!isUserLoggedIn()) {
                Toast.makeText(this, "Vui lòng đăng nhập để bookmark", Toast.LENGTH_SHORT).show()
                // TODO: Chuyển đến màn hình đăng nhập nếu cần
    ...
    private fun toggleBookmark() {
        Log.d("BOOKMARK_DEBUG", "=== TOGGLE BOOKMARK CLICKED ===")
        ...
        Log.d("BOOKMARK_DEBUG", "Button disabled, preparing request...")

        // KHẮC PHỤC LỖI: Loại bỏ readerId khỏi BookmarkToggleRequest vì nó được lấy từ Token.
        val request = BookmarkToggleRequest(
            novelId = novelId,
            bookmarkType = "Đã lưu" // Sửa lại theo yêu cầu của bạn
        )

        Log.d("BOOKMARK_DEBUG", "Request: novelId=$novelId, type=Đã lưu")
        Log.d("BOOKMARK_DEBUG", "Making API call...")

        bookmarkApiService.toggleBookmark(request)
            .enqueue(object : Callback<Map<String, Any>> {
                override fun onResponse(
                    call: Call<Map<String, Any>>,
                    response: Response<Map<String, Any>>
                ) {
                    Log.d("BOOKMARK_DEBUG", "=== API RESPONSE RECEIVED ===")
                    Log.d("BOOKMARK_DEBUG", "Response code: ${response.code()}")
                    Log.d("BOOKMARK_DEBUG", "Response success: ${response.isSuccessful}")

                    fabBookmark.isEnabled = true

                    if (response.isSuccessful) {
                        val data = response.body()
                        Log.d("BOOKMARK_DEBUG", "Response body: $data")

                        // Lấy action (added/removed)
                        val action = data?.get("action") as? String
                        val message = data?.get("message") as? String ?: ""

                        // Lấy số lượng bookmark (Nếu server trả về Double, cần ép kiểu)
                        val bookmarkCount = (data?.get("BookmarkCount") as? Double)?.toLong()
                            ?: (data?.get("bookmarkCount") as? Double)?.toLong() ?: 0L

                        val newIsBookmarked = action == "added"

                        Log.d("BOOKMARK_DEBUG", "Action: $action")
                        Log.d("BOOKMARK_DEBUG", "BookmarkCount: $bookmarkCount")

                        // Cập nhật UI
                        updateBookmarkIcon(newIsBookmarked)
                        updateBookmarkCountDisplay(bookmarkCount)
                        ...
                        Log.e("BOOKMARK_DEBUG", "Response error code: ${response.code()}")
                        Log.e("BOOKMARK_DEBUG", "Response error message: ${response.message()}")
                        Log.e("BOOKMARK_DEBUG", "Error body: $errorBody")
                ...
                override fun onFailure(call: Call<Map<String, Any>>, t: Throwable) {
                    Log.e("BOOKMARK_DEBUG", "=== API CALL FAILED ===")
                    Log.e("BOOKMARK_DEBUG", "Error: ${t.message}", t)
    ...
    // 🔹 FIXED: Load bookmark status với BookmarkStatusResponse
    private fun loadBookmarkStatus() {
        ...
        // KHẮC PHỤC LỖI: Bỏ tham số readerId vì nó được server lấy từ Token.
        bookmarkApiService.getBookmarkStatus(novelId)
            .enqueue(object : Callback<BookmarkStatusResponse> {
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        val data = response.body()
                        if (data != null) {
                            currentBookmarkId = data.bookmarkId
                            updateBookmarkIcon(data.isBookmarked)
                        }
                    } else {
                        Log.e("BookmarkError", "Load status failed: ${response.code()}")
                        updateBookmarkIcon(false)
                    }
                }

                override fun onFailure(call: Call<BookmarkStatusResponse>, t: Throwable) {
                    Log.e("BookmarkError", "Failed to load status: ${t.message}", t)
                    updateBookmarkIcon(false)
                }
            })
    }

    // 🔹 FIXED: Update bookmark icon
    private fun updateBookmarkIcon(bookmarked: Boolean) {
        isBookmarked = bookmarked

        val iconRes = if (bookmarked) {
            ICON_BOOKMARK_FILLED // Icon đã bookmark
        } else {
            ICON_BOOKMARK_OUTLINE // Icon chưa bookmark
        }

        val colorRes = if (bookmarked) COLOR_BOOKMARKED else COLOR_UNBOOKMARKED

        fabBookmark.setImageResource(iconRes)
        fabBookmark.setColorFilter(ContextCompat.getColor(this, colorRes))
    }

    // 🔹 NEW: Update bookmark count display
    private fun updateBookmarkCountDisplay(count: Long) {
        // Cập nhật TextView hiển thị số bookmark
        val currentText = tvNovelViews.text.toString()
        // Tách chuỗi hiện tại (ví dụ: "10K views • 500 bookmarks")
        val parts = currentText.split("•")
        if (parts.size >= 2) {
            // Lấy phần views (phần tử đầu tiên)
            val viewsPart = parts[0].trim()
            // Ghép lại với số bookmark mới
            tvNovelViews.text = "$viewsPart • ${formatNumber(count)} bookmarks"
        } else {
            // Trường hợp không có format views
            tvNovelViews.text = "${formatNumber(count)} bookmarks"
        }
    }
    ...
    private fun loadComments() {
        lifecycleScope.launch {
            try {
                ...
                // SỬA LỖI: Dùng commentApiService (đã xác thực)
    ...
    // ... (giữ nguyên các method còn lại: postComment, selectImage, onActivityResult,
    //      likeComment, dislikeComment, showEditCommentDialog, showDeleteConfirmDialog,
    //      editComment, deleteComment, displayNovelDetail, displaySynopsisTruncated,
    //      toggleSynopsisExpansion, displayGenres, createRoundedBackground, displayTags,
    //      formatNumber, showError, showRatingDialog)
    ...
    private fun submitRating(value: Int) {
        ...
        // SỬA LỖI: Dùng getAuthenticatedClient() vì Rating cần xác thực
        val api = ApiClient.getAuthenticatedClient().create(com.sinhvien.webnovelapp.api.RatingApi::class.java)
        ...
        api.createOrUpdateRating(rating).enqueue(object : retrofit2.Callback<Map<String, Any>> {
                ...
                if (response.isSuccessful) {
                    ...
                    when (response.code()) {
                        401 -> {
                            ...
                            Log.e("RatingError", "Response code: ${response.code()}")
                            Log.e("RatingError", "Response body: ${response.errorBody()?.string()}")
                            Toast.makeText(
                                this@NovelDetailActivity,
            ...
            override fun onFailure(call: retrofit2.Call<Map<String, Any>>, t: Throwable) {
                progressBarComments.visibility = View.GONE
                Log.e("RatingError", "Network error: ${t.message}", t)
                Toast.makeText(
                    this@NovelDetailActivity,
    ...
    private fun loadNovelRating() {
        // SỬA LỖI: Dùng getClient() vì tải Rating là công khai
        ...
        api.getAverageByNovel(novelId).enqueue(object : retrofit2.Callback<Map<String, Any>> {
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    val data = response.body()
                    val avg = (data?.get("averageRating") as? Number)?.toDouble() ?: 0.0
                    val total = (data?.get("totalRatings") as? Number)?.toInt() ?: 0
            ...
            override fun onFailure(call: retrofit2.Call<Map<String, Any>>, t: Throwable) {
                Log.e("RatingError", "Failed to load rating: ${t.message}")
                tvNovelRating.text = "★ 0.0 (0 ratings)"
            }
        })
    }
