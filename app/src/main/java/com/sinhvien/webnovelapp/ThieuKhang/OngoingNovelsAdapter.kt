...
class OngoingNovelsAdapter(
    private val onNovelClick: (Novel) -> Unit
) : ListAdapter<Novel, OngoingNovelsAdapter.OngoingNovelViewHolder>(NovelDiffCallback()) {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): OngoingNovelViewHolder {
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.item_novel_grid, parent, false)
        return OngoingNovelViewHolder(view, onNovelClick)
    }

    override fun onBindViewHolder(holder: OngoingNovelViewHolder, position: Int) {
        holder.bind(getItem(position))
    }

    class OngoingNovelViewHolder(
        itemView: View,
        private val onNovelClick: (Novel) -> Unit
        ...
        fun bind(novel: Novel) {
            ...
            when (novel.Status.lowercase()) {
                ...
                else -> tvStatus.setBackgroundResource(R.drawable.bg_status_ongoing)
            }
